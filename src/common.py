...
def validate_feature_columns(
    frame: pd.DataFrame, *, require_email: bool = False
) -> None:
    ...
    if non_numeric:
        raise ValueError(
            "Feature column(s) must contain numeric values: " + ", ".join(non_numeric)
        )
    if frame.loc[:, FEATURE_NAMES].isna().any().any():
        ...
        raise ValueError(
            "Feature column(s) contain missing values: " + ", ".join(missing_values)
        )
